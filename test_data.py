...
for file in file_list:
    ...
    print(df.isna().mean())
    if all(x in df.columns for x in pred_vars):
        df = df[pred_vars]
...
for file in file_list:
    print(file)
    test_df_norm = ut.normalize_test(test_all[file], scalar)
    print(f'{file} normalized')
    
    #Train and test data division
    test_x = np.asarray(test_df_norm.loc[:, test_df_norm.columns != response_var])
    ...
    pred = ut.invTransform(scalar, pred, "DTSM", test_df_norm.columns)
